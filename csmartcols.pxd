cdef extern from "libsmartcols.h":
    ...
    int scols_cell_set_data(libscols_cell* cell, const char* s)
    const char* scols_cell_get_color(const libscols_cell* cell)
    int scols_cell_set_color(libscols_cell* cell, const char* color)
    int scols_cell_get_flags(libscols_cell* cell)
    int scols_cell_set_flags(libscols_cell* cell, int flags)
    ...
    int scols_column_set_flags(libscols_column* column, int flags)
    ...
    int scols_column_set_whint(libscols_column* column, double whint)

    cdef struct libscols_line:
        pass
    libscols_line* scols_new_line()
    void scols_unref_line(libscols_line* line)
    int scols_line_add_child(libscols_line* line, libscols_line* child)
    libscols_cell* scols_line_get_column_cell(libscols_line* line, libscols_column* column)
    int scols_line_set_column_data(libscols_line* line, libscols_column* column, const char* data)
    const char* scols_line_get_color(libscols_line* line)
    int scols_line_set_color(libscols_line* line, const char* color)

    cdef struct libscols_symbols:
        pass
    libscols_symbols* scols_new_symbols()
    void scols_unref_symbols(libscols_symbols* symbols)
    int scols_symbols_set_branch(libscols_symbols* symbols, const char* s)
    ...
    int scols_table_print_range_to_string(libscols_table* table, libscols_line* start, libscols_line* end, char** data)
    ...
    int scols_table_add_line(libscols_table* table, libscols_line* line)
    int scols_table_enable_ascii(libscols_table* table, bint value)
    bint scols_table_colors_wanted(libscols_table* table)
    int scols_table_enable_colors(libscols_table* table, bint value)
